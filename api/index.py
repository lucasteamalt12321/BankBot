...
<div style="background:var(--bb-panel);border:1px solid var(--bb-primary);border-radius:16px;padding:24px;">
  ...
  <div class="row" style="margin-top:8px;">
    ...
    <button id="t-next" style="flex:1;padding:9px 16px;border-radius:10px;border:1px solid var(--bb-primary);background:var(--bb-primary);color:#fff;cursor:pointer;font-size:14px;margin-top:10px;">→</button>
  </div>
</div>
<script>
const T_DATA = __TERMS_DATA__;
const T_AUTH = localStorage.getItem('web_token') || '';
let tProg = {};
try { tProg = JSON.parse(localStorage.getItem('history_terms_prog') || '{}'); } catch(e) { tProg = {}; }
let tPushTimer=null;
function tSave(){ localStorage.setItem('history_terms_prog', JSON.stringify(tProg)); }
function tPush(){
  tSave();
  if(!T_AUTH) return;
  if(tPushTimer) clearTimeout(tPushTimer);
  tPushTimer=setTimeout(function(){
    fetch('/api/study/progress',{method:'POST',headers:{'Content-Type':'application/json','X-Auth-Token':T_AUTH},body:JSON.stringify({module:'history',cards:tProg})}).catch(function(){});
  },600);
}
function tRec(k, correct){
  const r = tProg[k] || {reps:0,interval:0,ease:2.5,correct:0,wrong:0,counter:0,streak:0,due:Date.now()};
  if(correct){
...
Object.keys(T_DATA.categories).forEach(function(c){
  const o=document.createElement('option');o.value=c;o.textContent=c+' ('+T_DATA.categories[c].length+')';
  document.getElementById('t-cat').appendChild(o);
});
loadTCat();
if(T_AUTH){
  fetch('/api/study/progress',{headers:{'X-Auth-Token':T_AUTH}}).then(function(r){return r.ok?r.json():null;})
   .then(function(d){ if(d&&d.cards&&d.cards.history){ tProg=Object.assign({},d.cards.history); tSave(); tRender(); } })
   .catch(function(){});
}
</script>"""
...
def emperors_page():
    ...
    terms_json = json.dumps(
        {"by_id": {t.id: {"id": t.id, "category": t.category, "term": t.term,
                           "definition": t.definition} for t in _H_TERMS},
         "categories": _H_CATS()},
        ensure_ascii=False,
    )
...
MATH_PAGE_TEMPLATE = """<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<script>
const MATH = __MATH_DATA__;
const AUTH = localStorage.getItem('web_token') || '';
const MODULE = "math";

let prog = {};
try { prog = JSON.parse(localStorage.getItem('math_prog') || '{}'); } catch(e) { prog = {}; }
function saveLocal() { localStorage.setItem('math_prog', JSON.stringify(prog)); }
function loadServerProgress() {
  if (!AUTH) return;
  fetch('/api/study/progress', { headers: { 'X-Auth-Token': AUTH } })
    .then(function(r){ return r.ok ? r.json() : null; })
    .then(function(d){ if (d && d.cards && d.cards.math) { prog = Object.assign({}, d.cards.math); saveLocal(); } })
    .catch(function(){});
}
let pushTimer = null;
function push() {
  ...
  pushTimer = setTimeout(function() {
    fetch('/api/study/progress', {
      ...
      body: JSON.stringify({ module: 'math', cards: prog })
    }).catch(function(){});
  }, 600);
}
...
Object.keys(MATH.topics).forEach(function(tp){
...
let tList = MATH.tasks.slice(), tIdx = 0;
function loadTasks(){ const tp=tSel.value; tList = tp ? MATH.tasks.filter(function(x){return x.topic===tp;}) : MATH.tasks.slice(); tIdx=0; renderTask(); }
function renderTask(){
...
// ---- Generators ----
const GEN = {
  g01: function(){ const p=5*Math.floor(1+Math.random()*9); const n=Math.floor(100+Math.random()*900); const a=Math.round(n*p/100); return {q:'Сколько составляет '+p+'% от '+n+'? (целое)', a:String(a), ex:'n*p/100 = '+n+'*'+p+'/100 = '+a}; },
  g02: function(){ const v=Math.floor(40+Math.random()*60); const t=Math.floor(2+Math.random()*6); const a=v*t; return {q:'Поезд едет '+v+' км/ч. Какое расстояние он пройдёт за '+t+' ч?', a:String(a), ex:'v*t = '+v+'*'+t+' = '+a}; },
  g03: function(){ const a=Math.floor(2+Math.random()*5), b=Math.floor(3+Math.random()*6); const r=1/(1/a+1/b); const ans=Math.round(r*100)/100; return {q:'Один мастер делает работу за '+a+' ч, другой за '+b+' ч. За сколько часов сделают вместе? (округли до 0.01)', a:String(ans), ex:'1/(1/'+a+'+1/'+b+') ≈ '+ans}; },
  g04: function(){ const k=Math.floor(Math.random()*3); if(k===0){const x=Math.floor(3+Math.random()*12),y=Math.floor(3+Math.random()*12);return{q:'Площадь прямоугольника '+x+'×'+y+'.',a:String(x*y),ex:x+'*'+y};} if(k===1){const b=Math.floor(4+Math.random()*14),h=Math.floor(3+Math.random()*12);return{q:'Площадь треугольника: основание '+b+', высота '+h+'.',a:String(b*h/2),ex:b*h/2};} const r=Math.floor(2+Math.random()*10);return{q:'Площадь круга радиусом '+r+' (π≈3.14).',a:String(Math.round(3.14*r*r)),ex:'3.14*'+r+'^2 = '+Math.round(3.14*r*r)}; },
  g05: function(){ const c=Math.floor(10+Math.random()*15), a=Math.floor(3+Math.random()*Math.min(c-3,12)); const b=Math.round(Math.sqrt(c*c-a*a)*100)/100; return {q:'Прямоугольный треугольник: гипотенуза '+c+', катет '+a+'. Найдите второй катет.', a:String(b), ex:'√(c²-a²) = '+b}; },
  g06: function(){ const x1=Math.floor(Math.random()*10),y1=Math.floor(Math.random()*10),x2=Math.floor(Math.random()*10),y2=Math.floor(Math.random()*10); const dx=x2-x1,dy=y2-y1; const d=Math.round(Math.sqrt(dx*dx+dy*dy)*100)/100; return {q:'Расстояние между ('+x1+','+y1+') и ('+x2+','+y2+').', a:String(d), ex:'√('+dx+'²+'+dy+'²) = '+d}; },
  g07: function(){ const a1=Math.floor(1+Math.random()*10), d=Math.floor(2+Math.random()*9), n=Math.floor(4+Math.random()*7); const an=a1+(n-1)*d; return {q:'Арифметическая прогрессия: a1='+a1+', d='+d+'. Найдите a'+n+'.', a:String(an), ex:'a1+(n-1)d = '+a1+'+'+((n-1)*d)+' = '+an}; },
  g08: function(){ const m=Math.floor(1+Math.random()*5), total=m+Math.floor(1+Math.random()*6); const g=gcd(m,total); return {q:'В корзине '+total+' шаров, из них '+m+' красных. Вероятность достать красный (в виде дроби a/b)?', a:(m/g)+'/'+(total/g), ex:m+'/'+total+' = '+(m/g)+'/'+(total/g)}; }
};
function gcd(a,b){ while(b){ const t=b; b=a%b; a=t; } return a; }
const gList = document.getElementById('g-list');
MATH.generators.forEach(function(g){
...
let curGen=null;
function startGen(gid){ curGen=gid; const g=MATH.generators.find(function(x){return x.id===gid;}); document.getElementById('g-card').style.display=''; document.getElementById('g-name').textContent=g.name; document.getElementById('g-desc').textContent=g.description; genNew(); }
...
document.getElementById('e-start').onclick=function(){
  const tasks = shuffle(MATH.tasks.slice()).slice(0,15).map(function(t){ return {type:'task', ref:t}; });
  const gens = shuffle(MATH.generators.slice()).slice(0,5).map(function(g){ const p=GEN[g.id](); return {type:'gen', gid:g.id, q:p.q, a:p.a, ex:p.ex}; });
...
def math_page():
    ...
    from core.mathematics.formulas import FORMULAS, TASKS, GENERATORS, formulas_by_topic
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ach-sec-count { font-size: 12px; color: var(--bb-muted); font-weight: 400; }
        .ach { background: var(--bb-elev); border: 1px solid var(--bb-border); border-radius: 12px; padding: 12px; display: flex; gap: 10px; align-items: flex-start; transition: all 0.15s; }
        .ach.locked { border-style: dashed; }
        .ach.locked .icon { filter: grayscale(1); opacity: 0.7; }
        .ach.locked .name { color: var(--bb-muted); }
        .ach .icon { font-size: 26px; flex-shrink: 0; }
        .ach .name { font-weight: 600; font-size: 13px; color: var(--gh-text2); }
        .ach .desc { font-size: 12px; color: var(--bb-muted); margin-top: 2px; line-height: 1.4; }
        ...
        .mf-btn { background: var(--bb-elev); border: 1px solid var(--bb-border); color: var(--bb-muted); border-radius: 20px; padding: 6px 14px; cursor: pointer; font-size: 12px; font-family: inherit; transition: all 0.15s; }
        .mf-btn:hover { border-color: var(--bb-link); color: var(--bb-text); }
        .mf-btn.active { background: var(--bb-accent); color: #0f1420; border-color: var(--bb-accent); font-weight: 600; }
