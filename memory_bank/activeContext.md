...
- ✅ [OGE-BUG-6] ИИ-куратор оценивает время по пунктам плана, система парсит и показывает (раньше `minutes` = число карточек). `ruff` clean, `tests/unit/test_curator.py` 22 passed. **Не закоммичено/не задеплоено.**
- ✅ История: убраны вопросы, где ответ уже в формулировке (напр. «при каком правителе был Ельцин» — персона-правитель сама у себя). Добавлена `itemIsLeaky` + фильтр в колоду/flash/counter/onlyErrors страницы `/emperors`. JS проверен `node --check`. **Не закоммичено.**
- ✅ [TASK] Уведомления (toast) при выполнении ОГЭ-заданий на всех 5 ОГЭ-страницах — реализовано через общий виджет `OGE_CURATOR_WIDGET` (MutationObserver ловит появление `.ok`/`.bad` и показывает toast).
- ✅ [TASK] Плавающая кнопка 🤖 в углу всех ОГЭ-модулей — открывает мини-окно (сегодняшний план «План на день» + чат с ИИ-куратором). Виджет `OGE_CURATOR_WIDGET` (HTML+CSS+JS) добавлен во все 5 страниц (`/emperors`, `/math`, `/physics`, `/russian`, `/informatics`) перед `</body>`/`</html>`. **Не закоммичено/не задеплоено.**
- ✅ [BUG] `/stats`: все поля ОГЭ-аналитики показывались по нулям. Причина — после слияния `/analytics` в `/stats` остался вызов `renderOgeAnalytics(oge)`, но тело функции (ранее `renderStats` в удалённом `ANALYTICS_PAGE_TEMPLATE`) не перенесли → `ReferenceError` на каждой загрузке, поля оставались на дефолтных `0`/`–`. Восстановил `renderOgeAnalytics` (адаптировал под `total_unseen` и `forecast.new`). Задеплоено (коммит `acb10f2`).
- ✅ [TASK] Прогресс-бары ОГЭ-модулей сделаны 2-цветными: зелёный = выученные (mastered), цвет модуля = начатые/в процессе (started − mastered); остаток — серый фон. Применено в `/stats` (`s-modules`) и на хабе (`addOgeProgress`). В текст добавлено «· N в процессе». ast + node --check (обе страницы) OK. **Не закоммичено/не задеплоено.**

### 🔲 Осталось (бэклог)
- ✅ [TASK] Убрать раздел «Статистика ОГЭ» (`/analytics`): полное слияние в общую статистику `/stats` + удаление `/analytics`. Сделано: контент аналитики (серия, общий прогресс, сегодня, по предметам, прогноз 14 дней, слабые места ОГЭ) теперь рендерится на `/stats` из `data.oge` (`/api/stats` уже включает `_oge_stats_payload`). Удалён `ANALYTICS_PAGE_TEMPLATE` и роут `/analytics` заменён на `redirect("/stats")`; карточка «Аналитика ОГЭ» убрана с хаба; `OGE_MODULES["quiz"].url` → `/stats`. `ruff`/ast OK, JS `/stats` и виджета прошли `node --check`. **Не закоммичено/не задеплоено.**
  - ✅ [SUB] Прогноз повторений (`_oge_stats_payload`, api/index.py) теперь показывает в каждый из 14 дней **и `due` (повторы), и `new` (новые карточки)**. Принцип `new`: оставшиеся неизученные карточки (`total_unseen` = сумма `unseen` по модулям) равномерно распределяются по 14 дням, темп `pace = max(3, round(total_unseen/14))`, с учётом остатка (`remaining`). Поле `total_unseen` добавлено в ответ. На фронте `/stats` ячейка прогноза показывает две строки: 🔁 повторы и 🆕 новые (≤5 — синяя, >5 — красная, 0/0 — серая).
- ✅ [TASK] Добавить разбивку по темам (+ режим «все темы») в ОГЭ-модули, где её нет (История не имеет topic-селектора; math/physics/russian уже имеют). Сделано: `classify_topic` в `core/history/emperors.py` (эвристика по ключевым словам, 9 тем: Восстания и революции / Войны и сражения / Церковь и вера / Внешняя политика и территории / Культура, наука и просвещение / Реформы и управление / Экономика и быт / Династия и престол / Прочее). Поля `topic` добавлены в `history_data` (events/persons) и в `allItems`; фильтр `topicMatch` в `itemsInScope()` (термины всегда показываются). `<select class="topic-sel">` («Все темы» + 9 тем) добавлен в панели Квиз/Сопоставление/Хронология; `toggleTopic` синхронизирует все и сбрасывает колоду. Закоммичено `3ca946b`, задеплоено.
- ✅ [TASK] Селектор масштаба Истории: добавить варианты по эпохам/периодам (сейчас только «5 императоров» / «Все правители»; 7 epoch есть в `eraGroups()`). Сделано: опции «Эпоха: …» (7 эпох из `eraGroups()`) динамически добавлены во все `.scope-sel`; `activeRulerIds()`/`activeRulers()` и `studyPanel()` теперь понимают scope `era:<Имя>` (фильтр по `grp.ids`). Закоммичено `3ca946b`, задеплоено.
- ✅ [VERIFY] Family Budget Module (BGT) + VK Mini App: проверено по mb — уже полностью реализованы. Бэкенд: таблицы `_ensure_budget_tables` (families/family_members/budget_transactions/transaction_details/debts/payments + `linked_vk_accounts`), все `/api/budget/*` эндпоинты (status/create/join/transactions/delete/debts/pay/balance/vk/link/vk/status), `/family_budget` страница, бот-команды. Фронтенд `vk_mini_app/` (React+TS+Vite, 7 страниц + `api/budget.ts`): `npm install` + `npm run build` (tsc && vite) проходит без ошибок типов, бандл собран. Статус deliverables BGT 98/100 (остался `BGT-TEST` — ручное тестирование). Предсуществовавшие падения `test_bunker_profile_parser`/`test_mafia_profile_parser` (4 failed) — сейчас ПРОХОДЯТ (89 passed в наборе bunker/mafia). Деплой фронта VK Mini App требует VK App ID + `VK_MINI_APPS_TOKEN` (у пользователя).
- ✅ [TASK] Прогноз ОГЭ (на `/stats`, куда влит `/analytics`): показывать, сколько **новых карточек будет выучено** (достигнут SM-2 mastery, reps≥3), а не только введено. В `_oge_stats_payload` каждый день прогноза теперь несёт `learned` (накопительно) и `learned_delta`. Логика: по планировщику (rep1→+1д, rep2→+3д, rep3=mastered) карточка, введённая в день j, осваивается в день j+4 → `learned_delta[i] = introduced[i-4]`. На фронте `/stats`: легенда добавила «✅ выучено (накопит.)», в каждой ячейке прогноза — зелёная строка `N✅ (+delta)`; под заголовком — сводка «За 14 дней выучишь ≈ N новых карточек (из M новых всего)». ast + node --check OK. Закоммичено `ad72441`, задеплоено.
- ✅ [BUG] Достижения: было «230 достижений», должно быть 325 — оказался стадый билд на проде. Решено редеплоем актуального кода (`len(ACHIEVEMENTS)=325`). Последующие деплои (вкл. `acb10f2`) подхватили. Закрыто.
- ✅ [TASK] OGE-прогноз `/stats`: добавлена оценка «сколько дней до выучивания ВСЕХ новых карточек» — `days_to_learn_all = ceil(total_unseen / pace) + 4` (темп ввода + 4 дня на освоение последней партии по SM-2). Сводка дополнена «· чтобы выучить все новые: ~K дн.». Закоммичено `1ba7636`, задеплоено.
...
- ✅ [TASK] Отдельный Vercel-сервис аудио (вариант Б): проект `audio_service/` (копия core/music: midi_utils/audio_utils) задеплоен отдельным Vercel-проектом → `https://audioservice.vercel.app` (CORS `*`, эндпоинты `/api/music/*` зеркальны LTHub). LTHub `/music` теперь обращается к сервису через `MUSIC_API_BASE` (env `AUDIO_SERVICE_URL`, дефолт `https://audioservice.vercel.app`). Проверено живьём: `/health` ok, analyze MIDI (bpm 120/key Am), change_tempo 200 audio/midi, CORS preflight 204. Закоммичено `636f980`. План «потом А»: клиентская обработка в браузере (Web Audio) — пока не начато.
- ✅ [FIX] Карточка «Музыка» перенесена из свёрнутого блока «Бета-модули» в видимую сетку хаба (пользователь не видел её). Задеплоено `9689100`.
- ✅ [FIX] Кнопка «Анализировать» в `/music` не срабатывала (необработанное исключение при не-JSON ответе сервиса). Обработчики обёрнуты в try/catch, добавлены проверки `r.ok`/Content-Type, ошибки выводятся в `#err`. Карточка возвращена в блок «Бета-модули» (требование пользователя). Задеплоено `f01cf85`.
- ✅ [TASK] Аудит багов по всему проекту (запрос пользователя): параллельно проверил core/*, api/index.py, music-подсистему; починил конкретные критичные/высокие баги, тесты зелёные (1480 passed). Закоммичено `92b7f44`, задеплоено на `bank-bot-ruby.vercel.app`. Открытые архитектурные (`[BANK-BUG-10/11/12]`, `[SEC-BUG-5/6]`) оставлены на согласование.
- 📌 [TODO] Вариант А (клиентская обработка в браузере, Web Audio API + JS-MIDI) — отложено, делается после Б (по выбору пользователя «сначала Б, потом А»).
...
> ✅ **Бета-аудит 2026-08-10 — проверено и закрыто (2026-08-28):** все 9 пунктов плана фиксов подтверждены как уже реализованные в последующих коммитах (лимит кубиков D&D; атомарный `pop` монет в шахматах + TTL + try/catch `JSON.parse`; per-user session-токен и валидация `answer_index` в тривии; экранирование XSS и очистка `VERB_GEN_LOCK` в глаголах; авторизация DELETE семьи через `_family_verify_member`; UNIQUE-индекс `daily_prayer_log`; `safeUrl`+экранирование в каноне; регистрация форсирует `is_admin=FALSE`, сессия админа строго по серверному флагу). Дополнительно в этом сеансе устранена последняя дыра: сервер шахмат больше **не возвращает правильный ход при ошибке** (`api_chess_puzzle_check` отдаёт `move` только при `correct`), клиент показывает «Попробуйте другой ход». Закоммичено `1af992a`, задеплоено.
