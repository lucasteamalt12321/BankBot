...
### 🔄 В работе (2026-09-12): 👥 Система профилей и друзей
- [TASK] «добавь систему профилей и друзей». Выбранное через вопросник (одобрено):
  - Модель: **дружба через заявки** (двусторонняя, как VK/Steam).
  - Публичный профиль **`/u/<login>`**: базовое (имя/@логин/аватар-инициалы/дата регистрации) + статистика (монеты, серия, активные дни, активность по модулям, достижения-иконки) + **GD-статистика** (`gd_nickname` → `/api/gd/user/<nick>`). Без ОГЭ-готовности и без био/приватности (не выбраны).
  - UI: страница **`/friends`** (друзья/входящие/исходящие/поиск/топ недели) + блок на **`/account`** + ссылка с бейджем заявок в user-bar хаба + кнопки действий прямо на `/u/…`.
  - Соревновательность: **«Топ недели среди друзей»** (SUM actions за 7 дней по `web_activity_log`). Стена/комментарии НЕ входят.
  - Данные: таблицы `friend_requests` (pending-заявки, accept→удаление+вставка `web_friends`) и `web_friends` (две строки на пару (A,B)+(B,A), ON DELETE CASCADE) → `_ensure_social_tables` в `get_db_engine`. Auth через `_require_web_user()`. Публичный профиль без личных полей (email/telegram/hash не отдаём). Rate-limit заявок ~20/час.
  - Остальной план (детали 9 роутов/страниц) — в activeContext при реализации.
