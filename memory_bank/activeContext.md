...
- 🔲 [TASK] Селектор масштаба Истории: добавить варианты по эпохам/периодам (сейчас только «5 императоров» / «Все правители»).
- 🔲 [BUG] Достижения: на странице «230 достижений», а должно быть 325. Проверено: в текущем коде `len(ACHIEVEMENTS)=325` (база 250 + 75 OGE-достижений). Значит прод крутит старый билд → нужен редеплой актуального кода. **Требует подтверждения/деплоя.**
