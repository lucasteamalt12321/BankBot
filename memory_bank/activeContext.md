...
## 🔜 Итерация «ИИ-куратор + UX» ОГЭ-центра (2026-08-25) — ТЗ записано, реализация не начата
...
**Дизайн-решения:**
- БД: таблица `oge_daily_plans` (user_id PK, plan_date, target_minutes, items_json, done_count, created_at) + `oge_chat_messages` (id, user_id, role user/assistant, content, created_at). DDL по паттерну `_ensure_` + вызов из блока создания движка.
- API: `GET /api/study/plan` (возвращает сегодняшний план; если нет — генерирует сразу = «первый заход»; учитывает вчерашний процент выполнения для облегчения), `POST /api/study/plan {target_minutes}` (перегенерация при смене времени; тот же день+те же минуты → отдаём сохранённый), `POST /api/study/chat {message}` + `GET /api/study/chat` (последние ~30 сообщений), прогресс дня — счётчик done в плане (инкремент при отметке выполнения пункта) + агрегат из `study_progress`.
- Промпт куратора: системная роль «методист ОГЭ», статистика из `_oge_subjects_payload`, бюджет минут, вчерашний результат, последние N сообщений чата.
- Фолбэк при недоступности Groq — правило-планировщик (как recommendations), чтобы чат/план не умирали.

### 🐛 Баги (записаны 2026-08-25 по протоколу пользователя)
- **[EXAM-BUG-1] Сводный экзамен не запускается** (аноним, other): `/exam` — экзамен не стартует. Мой свежий код OGE-06 — чинить первым делом (диагностика: локальный рендер + `node --check`, `GET /api/exam/mixed?n=5` без auth — возможно, падает на пустом uid или JS-ошибка старта).
- **[AICHAT-BUG-1] AI Chat «Ошибка AI: 404»** (аноним, other): страница AI Chat, персонаж 🍵 Чай («Говори вдохновляюще… настой/eight-nine/кружка-алтарь») → «❌ Ошибка AI: 404». Гипотеза: `call_ai_api` (api/index.py:2821) бьётся в несуществующую модель/endpoint провайдера (404). Проверить модель Groq (llama-3.3-70b-versatile?), ключ, URL; влияет на ВСЕ ИИ-фичи (ai-plan молча падает в fallback). Починить до реализации OGE-08.
