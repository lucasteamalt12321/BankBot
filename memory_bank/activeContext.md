...
## Текущий фокус
**Стабилизация деплоя на Hugging Face Spaces.** Основная сетевая проблема решена через IP-based proxy (`195.201.225.248`) с кастомным `Host` header и отключённой SSL verification. Остаётся валидация полного цикла: startup → polling → ответ на команды в HF.
