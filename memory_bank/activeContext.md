...
- 🔲 [TASK] Добавить разбивку по темам (+ режим «все темы») в ОГЭ-модули, где её нет (История не имеет topic-селектора; math/physics/russian уже имеют).
- 🔲 [TASK] Селектор масштаба Истории: добавить варианты по эпохам/периодам (сейчас только «5 императоров» / «Все правители»; 7 epoch есть в `eraGroups()`).
