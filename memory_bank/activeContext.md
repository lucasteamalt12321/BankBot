...
### 🏁 Выполнено (2026-09-26): 🔐 Регистрация — школьный класс за NAT больше не блокируется (коммит `c521361`)

> Закрыт последний пункт бэклога-продуктовых кандидатов (лимит регистрации 5/IP за 300 с). Указание пользователя: «да» → продолжать по memory bank.

- **[BUG] лимит регистрации 5/IP за 300 с блокировал школьный класс за одним NAT-IP** (6-й ученик получал 429). `api_auth_register` использовал только in-memory `_check_ai_rate("register:<ip>", 5, 300)` — счётчик к тому же ещё и обнуляется на каждом холодном старте Vercel, т.е. реальной защиты почти не давал, а легитимным пользователям мешал.
- **Фикс (двухуровневый лимит):** burst `_REGISTER_BURST = 30` регистраций с IP за 5 минут (in-memory, весь класс проходит) + `_REGISTER_DAILY = 200` за 24 часа через `_check_db_rate("reg_day:<ip>", …, 86400)` — жёсткий backstop, переживает холодные старты и режет массовые регистрации ботов. Уникальность login/email осталась как раньше.
- **Тесты:** `test_register_rate_limits` в `test_web_portal_e2e.py` — 30 регистраций подряд с одного IP → все 200, 31-я → 429 (burst); заполненный `rate_limits` на 200 записей → 429 с текстом «…за сутки…» даже при чистом burst-счётчике. Регресс 64 passed (+ 2 pre-existing падения `test_canon_requests_e2e`, подтверждены на чистом HEAD), ruff чист.
...
- **Фикс:** GD-ALTER-ы (levels.difficulty, submissions.difficulty/attempts, player_stats.points/demons_count, level_completions.player_name) переведены на идемпотентный `_ddl_add_column(...)` (проверяет `information_schema.columns`/PRAGMA до ALTER, возвращает None когда колонка есть — ALTER вообще не выполняется); то же самое для `emperors_progress.counter` (тот же латентный баг). `_ddl_add_column` определяется выше (стр. ~244), Доступен в `_ensure_gd_tables`.
- **Проверка:** ruff clean; sqlite-прогон `_ensure_gd_tables/_ensure_emperors_tables/_ensure_parsing_tables/_ensure_code_tables` дважды каждый → idempotent OK (аналог «уже мигрировано» на проде); регресс code_explainer + freemium + error_logging 58 passed. 10 падений в `test_gd_commands.py` — **pre-existing**, не связаны: async-тесты, не установлен `pytest-asyncio` («async def functions are not natively supported»).
- Коммит `b6c342f` запушен (`cabfe45..b6c342f`), деплой `lthub-f5xqmu1bf-...` (Ready).

- **[TASK] «сделай чтобы при уведомлениях требуется регистрация была кнопка войти» → решение пользователя: «вместо некрасивого системного уведомления большой прямоугольник в верхней части экрана».** В `_FREEMIUM_AUTH_JS` (api/index.py): (1) fetch-патч — JSON-401 без `auth_required` → `window.ltBanner(js.error)` (модалка осталась для `{auth_required}`); (2) `ltBanner()` — большой фикс-топ баннер (тема `--bb-*`, кнопка **«Войти»** → открывает модалку входа, крестик закрытия, один переиспользуемый элемент); (3) `window.alert`-перехват для маркеров «авторизац/войдите/регистрац/требуется вход/нужно войти» → баннер (остальные alert() сохранены). Покрывает `showErr() + alert()` на /code (401 `/api/code/project/<id>/comment`) и любые `alert("Требуется авторизация…")`. Коммит `44f9cab`, деплой `lthub-7uptqdpe2-...` (Ready, на алиасе). Верификация: ruff clean, 37/37 тестов, `node --check` отрендеренного JS. **⚠️ Прод-смоук баннера блокирован**: песочница начала сбрасывать TCP к Vercel (TLS recv failure ~19,5с) — повторить из рабочей сети.
- **[TASK] «Предложения на потом» из прошлой GD-сессии — выполнены:**
  - **Освобождение аккаунтов:** удалены осиротевшие тестовые аккаунты проде `smoke2026fb/fc/fd/fe/fg/fh` (6 шт.) временным guarded-эндпоинтом (`CLEANUP_NONCE` env + header `X-Cleanup-Nonce`); падающая команда в общем `begin()` = `InFailedSqlTransaction` на Postgres (первый прогон 500) → раздельные транзакции по таблицам. Эндпоинт и env удалены после очистки.
  - **Фиксы `_ai_chat` (коммит `29e7a9c`):** «patience-breach» — фильтр `"gpt-oss" not in candidate` применялся к ВСЕМ провайдерам и молча пропускал Gemini-primary; `_groq_active_model` читался и для Gemini. Groq-специфика изолирована в ветку Groq; не-Groq зовётся одиночным POST (ретрай 429, пустой content → fallback Groq). Новые тесты `tests/unit/test_ai_chat_fallback.py` (5/5). На проде `GEMINI_API_KEY` не установлен — активируется добавлением ключа.
- Коммиты: `29e7a9c` (ai_chat fix + тесты + удаление cleanup-эндпоинта), `44f9cab` (UI-баннер) — **НЕ ЗАПУШЕНЫ** (pull --rebase перед push). Changelog + last_checked_commit → `44f9cab` в progress.md обновлены.
...
- **Осталось в очереди (не сделано в этой сессии):** баг Code Explainer — (1) кнопка «Анализировать всё» не работает: ложный «Прогресс не растёт (лимит ИИ?)» из-за сравнения `analyzed_count`, жёсткий `_CODE_MAX_FILES = 35`, rate-limit 10/час против `series >= 12`; (2) качество комментариев — «Файл html, 20 строк» вместо содержательных (`_code_analyze_file_ai` prompt + fallback-комментарии `_code_analyze_batch`). Также продуктовый кандидат: лимит регистрации 5/IP за 300 с блокирует школьный класс за одним NAT-IP.
  - **→ [BUG 1+2 закрыты коммитом `c832c95` (2026-09-26), см. запись вверху. Продуктовый кандидат «лимит регистрации 5/IP за 300 с» закрыт коммитом `c521361` (burst 30/5мин + суточный 200/IP), см. запись вверху.]**

### ✅ Выполнено (2026-09-24): 💻 Code Explainer — гостевой скан без больших файлов (коммит `c97ad4f`)

- **[TASK] «в code explainer без регистрации можно загрузить проект, но нельзя просканировать. Может разрешим просканнировать, но без больших файлов?»:** анонимам разрешён «🚀 Анализировать всё» ТОЛЬКО для гостевых проектов (user_id NULL), **без больших файлов**. `_api_code_reanalyze`: для гостей из пула исключаются файлы `line_count > _CODE_GUEST_SCAN_MAX_LINES=800` (`skipped_large` в ответе), rate-limit `code_scan_anon_{ip}` 3/час + `_check_ai_rate`; авторизованные анализируют как раньше. UI `/code`: если `skipped_large > 0` → «(большие файлы пропущены — войдите, чтобы сканировать всё)».
- Тесты `test_code_explainer.py` +3 → 13/13; регресс 49 зелёных, ruff clean. Коммит `c97ad4f` **запушен** (`e586262..c97ad4f`). Деплой `lthub-dsesijizn-lucasteamalt12321s-projects.vercel.app` (Ready).
- **Прод-смоук OK** (через `--resolve lthub.vercel.app:443:64.29.17.195`): аноним loadGit octocat/Hello-World → 200; аноним reanalyze → **200** (раньше 401); lodash → `{file_count:40, skipped_large:15, analyzed_count:3}` — большие файлы исключены реально.
- **Диагноз сети (откл. песочницы к Vercel):** локальный DNS отдаёт edge-IP `.67` (недоступны, TCP/TLS reset), рабочие `.195`; в WinHTTP мёртвый прокси `127.0.0.1:10801` (SYN_SENT). Сама страница жива (200). Подсказка для пользователя: hosts-pin `.195` + снять/перезапустить прокси.


### ✅ Выполнено (сессия 2026-09-22): 🎛 Freemium-гейтинг 67/33 во всех модулях — 100%
...
- План в `memory_bank/projectbrief.md` → Phase 9 (FRE-01..FRE-09), **закрыта полностью: Phase 9: 100/100**. Итог: `_auth_user_or_401()` + единый 401 `{auth_required}` + глобальный фронт-патч `_FREEMIUM_AUTH_JS` (модалка логина, SUPPRESS-список); раскрыта база code (analyze+гостевые проекты), dnd, chess анонимам с rate-limit; гейты md2pdf-ИИ, music-overlay, study-ИИ/chat/hint/stats, exam ai-record, verbs submit, trivia answer (+1 монета); регистрация без email (NULL) + бонус 100 монет + `created_via`.
- **Прод-фиксы по ходу:** `code_projects.user_id` NOT NULL → nullable (гостевой analyze 500); email `''` → NULL; `log_error` вешал тесты (Telegram без сети) → conftest autouse no-op; **прод-регистрация 500** из-за SQLite-синтаксиса (`AUTOINCREMENT`/`DEFAULT NOW()`) в `_ensure_web_coin_tables` → каноническая Postgres-DDL `SERIAL + DEFAULT CURRENT_TIMESTAMP` (портабельна и для SQLite-тестов) → **prod register без email → 200, login 200, монеты +100**.
- Коммиты: `f6132eb` (фичи/gates/тесты), `64740ad` (docs(mb)), `8fa39f2` (фикс DDL регистрации) — все запушены; деплой `lthub-hin4fiiqw-lucasteamalt12321s-projects.vercel.app`. Регрессия 32/32, ruff clean. Changelog + last_checked_commit обновлены.
