...
### 🟦 В работе (в этой сессии, 2026-09-20): 📄 Markdown → PDF (`/md2pdf`)
- **[TASK] «новый модуль: md to pdf. экран разделен на 2 части: с одной стороны человек вставляет гипертекст, а с другой он же но pdf. и кнопка скачать. типа markdowntopdf.com»** — SPA страница: слева textarea Markdown, справа живое превью в виде «PDF-листа» (A4), кнопка «⬇️ Скачать PDF». Рендер клиент-сайд (marked.js + highlight.js с CDN, по образцу `/code`). Download = браузерный print-to-PDF (по выбору пользователя): скрытый iframe с чистым print-HTML (`@page A4`, печатные стили) → `iframe.contentWindow.print()`, фолбек `window.print()`. localStorage-автосохранение черновика. Карточка в beta-секции хаба рядом с 🔍:/code.
