...
| ID | Deliverable | Status | Weight |
|----|-------------|--------|--------|
| OGE-08 | ИИ-куратор: клик по «Плану на день» открывает чат; промпт = прогресс из карточек + слабые предметы/темы + дневной бюджет времени + история переписки из БД (прочерк если пусто); ответы сохраняются в БД | pending | 9 |
| OGE-09 | Персистентный план на день: генерация один раз в сутки (первый заход в хаб), перегенерация только при смене целевого времени (чипы 5/10/15/20/30 мин), облегчение плана если вчера не справился, остаток дня — мониторинг прогресса | pending | 8 |
