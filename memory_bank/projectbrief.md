...
### Phase 6: OGE Center — подготовка к ОГЭ по 5 предметам (2026-08-24)

**Контекст:** у пользователя ОГЭ через год по 5 предметам: математика, русский, информатика, физика, история. Система должна сама проверять прогресс и предлагать, что пройти актуально. Детальный план: `memory_bank/oge_center_plan.md`. Контент — сначала MVP (~50–100 позиций/предмет), затем добить весь кодификатор ФИПИ (OGE-FULL).

| ID | Deliverable | Status | Weight |
|----|-------------|--------|--------|
| OGE-00 | Ядро: единая таблица `study_progress` + API GET/POST/reset `/api/study/progress` + планировщик `/api/study/recommendations` (просроченные → слабые → новые, вес по датам ОГЭ) + хаб-виджет «План на сегодня» с бейджами + идемпотентная миграция `emperors_progress` + переименование информатики `/math`→`/informatics` (redirect) | completed | 14 |
| OGE-01 | Математика `/math`: `core/mathematics/` — карточки формул + генераторы параметрических задач + экзамен-режим | completed | 13 |
| OGE-02 | Информатика: расширение `core/informatics/` до 19 тем × 5 = 95 задач; подключение серверного прогресса | completed | 9 |
| OGE-03 | История: `core/history/` — TERMS (100 терминов ОГЭ по 5 периодам), встроены во вкладку `/emperors`; `/terms` → redirect | completed | 7 |
| OGE-04 | Русский `/russian`: `core/russian/` — карточки правил + тестовые задания формата ОГЭ + чек-листы сочинения по критериям ФИПИ | completed | 10 |
| OGE-05 | Физика `/physics`: `core/physics/` — шпаргалка формул, карточки «формула↔величина↔единица СИ», расчётные задачи + генераторы | completed | 9 |
| OGE-06 | Полировка: сводный экзаменатор `/exam` (микс предметов, серверная проверка, SM-2-запись), статистика слабых тем на страницах | completed | 6 |
| OGE-FULL | Волна контента FULL: математика m31–m50, русский r41–r50/t31–t45, физика f58–f62/t29–t40, информатика lesson16–19, история t81–t100 | completed | 3 |
| OGE-07 | ИИ-план дня (`/api/study/ai-plan`, Groq, кэш на день) + переключатель режима ОГЭ в хабе | completed | 2 |

#### Итерация «ИИ-куратор + UX» (2026-08-25)

| ID | Deliverable | Status | Weight |
|----|-------------|--------|--------|
| OGE-08 | ИИ-куратор: клик по «Плану на день» открывает чат; промпт = прогресс из карточек + слабые предметы/темы + дневной бюджет времени + история переписки из БД (прочерк если пусто); ответы сохраняются в БД | completed | 9 |
| OGE-09 | Персистентный план на день: генерация один раз в сутки (первый заход в хаб), перегенерация только при смене целевого времени (чипы 5/10/15/20/30 мин), облегчение плана если вчера не справился, остаток дня — мониторинг прогресса | completed | 8 |
| OGE-10 | Автопроверка вместо «Знаю/Не знаю»: система сама проверяет ответы (формулы/правила — выбор варианта, термины — ввод текста с нормализацией) во всех предметах | pending | 5 |
| OGE-11 | История `/emperors`: термины — вкладкой НА УРОВНЕ Тренажёр/Экзамен/Сопоставление/Хронология; режимы тренажёра: имена, события, термины, все сразу | pending | 3 |
| OGE-12 | ИИ сам предлагает выбор темы/режима везде, где можно (баннеры-подсказки на предметных страницах и в хабе) | pending | 2 |

**OGE Center: 90/100** (Phase 6 база завершена: ядро, 5 предметов, контент FULL, ИИ-план, куратор-чат, персистентный план; OGE-08/09 закрыты; веса пересчитаны 2026-08-25)
