...
| ID | Deliverable | Status | Weight |
|----|-------------|--------|--------|
| OGE-00 | Ядро: единая таблица `study_progress` + API GET/POST/reset `/api/study/progress` + планировщик `/api/study/recommendations` (просроченные → слабые → новые, вес по датам ОГЭ) + хаб-виджет «План на сегодня» с бейджами + идемпотентная миграция `emperors_progress` + переименование информатики `/math`→`/informatics` (redirect) | completed | 20 |
| OGE-01 | Математика `/math`: `core/mathematics/` — MVP ~130 карточек формул (сокращённое умножение, прогрессии, планиметрия, тригонометрия, вероятность) + генераторы параметрических задач (проценты, текстовые, координаты) + экзамен-режим | completed | 18 |
| OGE-02 | Информатика: расширение `core/informatics/` (бывш. core/math) с 9 до ~17 тем ОГЭ (системы счисления + генератор переводов, кодирование информации, логика, графы, электронные таблицы, файлы/сети) → ~120 задач; подключение серверного прогресса | completed | 12 |
| OGE-03 | История: дополнение `core/history/` — TERMS (~80 терминов ОГЭ), культура, режим «термин↔определение», переход на общий движок прогресса | completed | 10 |
| OGE-04 | Русский `/russian`: `core/russian/` — карточки правил (НН/Н, НЕ/НИ, пунктуация ССП/СПП/ДСП, обороты) + тестовые задания формата ОГЭ + чек-листы изложения/сочинения по критериям ФИПИ (без автопроверки) | completed | 14 |
| OGE-05 | Физика `/physics`: `core/physics/` — шпаргалка формул по разделам (механика→квантовая), карточки «формула↔величина↔единица СИ» (matching), простые расчётные задачи | completed | 12 |
| OGE-06 | Полировка: экзамен-режимы во всех предметах (таймер, оценка по шкале ОГЭ), ачивки по предметам, статистика слабых тем на страницах | completed | 8 |
| OGE-FULL | Добить весь кодификатор ФИПИ по всем 5 предметам после MVP (зафиксировано пользователем как следующий шаг) | pending | 4 |
| OGE-07 | ИИ-план дня (`/api/study/ai-plan`, Groq, кэш на день) + переключатель режима ОГЭ в хабе (скрытие виджета и предметных карточек) | completed | 2 |

**OGE Center: 96/100 (MVP всех предметов + ИИ-план + режим ОГЭ готовы; OGE-FULL — глубокое расширение кодификатора, открыт) (OGE-00/01/02/04/07 завершены; далее Физика, История TERMS, Экзаменатор)**
