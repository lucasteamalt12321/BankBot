...
| OGE-10 | Автопроверка вместо «Знаю/Не знаю»: система сама проверяет ответы (формулы/правила — выбор варианта, термины — ввод текста с нормализацией) во всех предметах | completed | 5 |
| OGE-11 | История `/emperors`: термины — вкладкой НА УРОВНЕ Тренажёр/Экзамен/Сопоставление/Хронология; режимы тренажёра: имена, события, термины, все сразу | completed | 3 |
| OGE-12 | ИИ сам предлагает выбор темы/режима везде, где можно (баннеры-подсказки на предметных страницах и в хабе) | pending | 2 |

**OGE Center: 98/100** (Phase 6 база завершена: ядро, 5 предметов, контент FULL, ИИ-план, куратор-чат, персистентный план, термины-вкладки; OGE-08/09/10/11 закрыты)
