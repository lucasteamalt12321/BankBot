...
- **Проблема:** при полном недоступности всех ИИ-провайдеров `/api/study/chat` возвращал 502, и модалка куратора показывала «❌ Куратор временно недоступен».
- **Фикс 1 — устойчивость `call_ai_api`:** сетевой сбой на модели Groq больше не обрывает перебор кандидатов (continue вместо return); 429 также пробует следующую модель; ошибка парсинга успешного ответа основного провайдера уходит в Groq-фолбэк вместо мгновенного ❌.
- **Фикс 2 — rule-based фолбэк куратора:** новая `_curator_fallback_reply(subjects, plan_lines)` — при недоступности ИИ чат отвечает автоподсказкой (план дня + совет по слабой теме/повторению из `_oge_subjects_payload`), сообщения продолжают сохраняться в историю; 502 убран.
- **Проверки:** ruff clean, py_compile OK; смоук-тесты локально падают по известной легаси-причине окружения (Python 3.14 против pinned 3.12, зависимости не собираются — чинит другой разработчик, ОГЭ не блокирует). Коммит `e79a988`, запушен.
- **Подтверждено по git-логу:** итерация OGE-08…12 уже реализована коммитами `53ed71f`, `64b1b16`, `e5d948b`, `7d84430`, `1553840`, `fca5b62`; оба приоритетных бага исправлены в `83082af`.
...
## last_checked_commit
e79a988 (2026-08-25; фикс куратора — rule-based фолбэк чата + устойчивый call_ai_api; ранее в этот день: реализация итерации OGE-08…12 — план дня/чат куратора 53ed71f+64b1b16, термины-вкладка и режимы тренажёра e5d948b, автопроверка 7d84430, ИИ-подсказки /api/study/hint 1553840, тесты fca5b62; фиксы EXAM-BUG-1/AICHAT-BUG-1 83082af)
### Задача 1. Ачивки
- **Статус:** Готово
