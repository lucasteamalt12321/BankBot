...
### 2026-09-12 (Session: 🧠 AI-чат — чекбоксы-контекст вместо вкладок)
- **[TASK]** Запрос пользователя (выбран вариант А, но «не вкладки, а чекбоксы»): объединить 3 AI-модуля (AI Chat / Куратор / Code Review) без единого переключателя-табов.
  - На `/ai_chat` добавлена панель `Контекст:` с двумя чекбоксами:
    - **Куратор** — встраивает персону ИИ-куратора (стиль Ольги Георгиевны, приоритеты, ссылки на модули) в системный промпт.
    - **ОГЭ-прогресс** — живой снимок: план на день, прогресс по предметам, слабые темы, серия/точность, обратный отсчёт, вчерашняя статистика (блок «ДАННЫЕ УЧЕНИКА (ОГЭ)»).
  - Бэкенд: `_ai_chat_context_block()`/`_oge_data_snapshot()`; `_pc_build_prompt`/`_pc_ai_chat` принимают `extra_context` (добавляется к системному сообщению).
  - Состояние чекбоксов в `localStorage.ai_chat_context`, default OFF; payload `/api/ai_chat` получает `context:{curator,oge}`.
  - Оба контекста ортогональны; для ОГЭ-прогресса нужен вход в аккаунт.
  - Проверки: `node --check` (JS-блок страницы) OK, `py_compile` OK, `ruff` OK. Задеплоено (Ready).
...
## last_checked_commit
4d640d9 (2026-09-12; feat(ai_chat): чекбоксы-контекст «Куратор» / «ОГЭ-прогресс» в промпт AI)
