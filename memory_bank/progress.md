...
> - 🐉 **DnD (AI Master) — выпущен из беты** (DND-BUG-1 исправлен: реализован шаринг сессий — `share_code` + `/api/dnd/join` + карточка «Сессия» с кнопкой копирования ссылки на `/dnd?session=<code>`).
