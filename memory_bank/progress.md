...
- **Реальная первопричина «не анализируется / кнопки не работают» (повторялось 6+ раз):** НЕ баг кода, а **закэшированная браузером/VK-webview старая страница `/music`**. Сервер `/api/music/analyze` стабильно отвечает 200 (проверено curl: bpm/key). Клиентский скрипт корректен (node --check + симуляция вызова обработчиков — OK). `no-store` в ответе Вебвью VK игнорирует, поэтому страница не обновлялась между деплоями. **Фикс:** версионированная ссылка `/music?v=<BUILD_TAG>` (BUILD_TAG = git-short-hash или `b<time.time()>` при недоступном git) + мета `Cache-Control: no-store, must-revalidate` / `Pragma: no-cache` на `/music` + `no-store` на `/`. Задеплоено. Коммит: <последний про музыку-кэш>.
- **Правило проекта:** для страниц, открываемых из VK Mini App / мобильного вебвью, всегда версионировать ссылки (query `?v=BUILD_TAG`) и ставить no-store — иначе пользователь годами видит старую копию и жалуется на «сломанные кнопки».
