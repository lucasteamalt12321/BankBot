...
class StartupValidator:
    ...
    def validate_env_file() -> bool:
        ...
        logger.error(
            "❌ env-файлы не найдены!\n"
            "Пожалуйста, создайте config/.env.local для секретов и при необходимости используйте config/.env.shared как коммитируемый слой настроек.\n"
    ...
    def validate_required_settings() -> Tuple[bool, List[str]]:
        ...
        if missing:
            logger.error(
                ...
                "Пожалуйста, проверьте config/.env.local и/или коммитируемые env-слои в config/.env.shared."
            )
