#!/usr/bin/env python3
"""
Script to automatically update config imports to use the new src.config module.
...
Usage:
    python scripts/fix_config_imports.py [--dry-run] [--path PATH]
...
# Additional replacements for specific imports that need mapping
SPECIFIC_REPLACEMENTS = {
    # Old config had these, new config has different structure
    'settings.bot_token': 'settings.BOT_TOKEN',
    'settings.database_url': 'settings.DATABASE_URL',
    'settings.admin_user_ids': '[settings.ADMIN_TELEGRAM_ID]',  # Changed from list to single ID
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Fix config imports to use src.config',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Preview changes for entire project
  python scripts/fix_config_imports.py --dry-run
  
  # Apply changes to entire project
  python scripts/fix_config_imports.py
  
  # Apply changes to specific directory
  python scripts/fix_config_imports.py --path bot/
  
  # Preview changes for specific file
  python scripts/fix_config_imports.py --dry-run --path bot/bot.py
        """
    )
