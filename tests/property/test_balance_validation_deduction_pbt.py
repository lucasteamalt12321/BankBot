        ...
        **Validates: Requirements 2.1, 2.2, 2.3**
        ...
        if expected_success:
            ...
            self.assertEqual(
                ...
                "Transaction should be logged for successful purchase"
            )

            # Verify purchase result contains new balance
            self.assertEqual(
                result["new_balance"], expected_final_balance,
                "Purchase result should contain correct new balance"
            )
            ...
            self.assertEqual(
                ...
                "No transaction should be logged for failed purchase"
            )

            # Verify new balance in result matches current balance
            self.assertEqual(
                result["new_balance"], initial_balance,
                "Failed purchase result should contain current balance"
            )
    ...
    def test_insufficient_balance_always_denies_purchase(self, telegram_id, insufficient_balance):
        ...
        **Validates: Requirements 2.1, 2.2**
        ...
        self.assertEqual(
            ...
            "Error code should indicate insufficient balance"
        )
