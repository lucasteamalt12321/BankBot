...
Текущий фикс: краткий режим обычных меню включён по умолчанию; `/short` и `/long` реализованы как краткий/полный режим для одного пользователя; `/short_all` и `/long_all` — для всех пользователей, включая обработку в группах через mentioned-команды.

По просьбе пользователя временно отключён режим часов (`ntfy`/`ADB` realtime), при этом команды `/short` и `/short_all` оставлены рабочими как краткий режим меню.
