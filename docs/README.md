...
- `api/index.py` — страница `/emperors` (вкладки «📚 Изучить» и «🧠 Тренажёр», счёт и список ошибок в localStorage, без БД).
- Карточка «Императоры России» в бета-блоке хаба `/`.

**API эндпоинты:** нет (страница полностью статическая, данные внедряются JSON-ом при рендере).
