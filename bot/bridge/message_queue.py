"""Message queue for Bridge module — rate-limited outbound message delivery."""
...
class MessageQueue:
    ...
    Example:
        >>> def my_sender(msg: OutboundMessage) -> None:
        ...     print(f"Sending: {msg.text}")
        >>> mq = MessageQueue(send_func=my_sender)
        >>> mq.put(OutboundMessage(text="Hello", platform="vk"))
        >>> mq.stop()
    """
    ...
    def _run(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                self._send_func(msg)
                backoff = 1.0  # сбросить при успехе
