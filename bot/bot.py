    ...
    async def game_turn_command(
        ...
        try:
            ...
            if session_info["game_type"] == "cities":
                ...
                result = games.process_killer_words_turn(
                    session_id, user.id, turn_input
                )
...
/turn {session_id} <ход>
                ...
                await update.message.reply_text(
                    f"{result.get('reason', 'Oshibka pri obrabotke khoda')}"
                )
        except Exception as e:
...
/dnd_character_create {session_id} <имя> <класс>
    ...
    async def dnd_sessions_command(
        ...
        try:
            ...
            if not sessions:
                await update.message.reply_text(
                    "📭 Вы не участвуете ни в одной D&D сессии"
                )
                return
    ...
    async def dnd_roll_command(
        ...
        try:
            ...
            if dice_type not in valid_dice:
                raise ValueError(
                    f"Неверный тип кубика. Доступные: {', '.join(valid_dice)}"
                )
...
"""
            for challenge in challenges["challenges"]:
                ...
                text += (
                    f"   📊 Прогресс: {challenge['progress']}/{challenge['target']}\n"
                )
    ...
    async def motivation_stats_command(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
...
"""
    ...
    async def achievements_command(
        ...
        try:
            ...
            available_achievements = achievement_system.get_available_achievements(
                user.id
            )
    ...
    async def notifications_command(
        ...
        try:
            ...
            notifications = notification_system.get_user_notifications(
                user.id, unread_only=False, limit=10
            )
                ...
                for req in friend_requests[:5]:  # Показываем первые 5
                    keyboard = [
                        [
                            InlineKeyboardButton(
                                "✅ Принять",
                                callback_data=f"friend_accept_{req['user_id']}",
                            ),
                            InlineKeyboardButton(
                                "❌ Отклонить",
                                callback_data=f"friend_reject_{req['user_id']}",
                            ),
                        ]
                    ]
                    reply_markup = InlineKeyboardMarkup(keyboard)
...
📅 Отправлен: {req["sent_at"].strftime("%d.%m.%Y %H:%M")}
                    ...
                    await update.message.reply_text(
                        request_text, reply_markup=reply_markup
                    )
                text += "\n"
            ...
            if friends:
                ...
                for friend in friends[:10]:  # Показываем первых 10
                    ...
                    text += f"• {friend_name}\n"
                    text += (
                        f"  {'@' + friend['username'] if friend['username'] else ''}\n"
                    )
    ...
    async def friend_accept_command(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        ...
        if not context.args or not context.args[0].isdigit():
            await update.message.reply_text(
                "❌ Используйте: /friend_accept <id_пользователя>"
            )
            return
    ...
    async def gift_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            social = SocialSystem(db)
            result = social.send_gift(
                user.id, receiver_identifier, "coins", amount, message
            )
...
"""
                for member in clan_info["members"][:10]:  # Показываем первых 10
                    role_icon = {"owner": "👑", "officer": "⭐", "member": "👤"}.get(
                        member["role"], "👤"
                    )
                    ...
                    text += (
                        f"   В клане с: {member['joined_at'].strftime('%d.%m.%Y')}\n\n"
                    )
...
/clan_leave - покинуть клан (если вы владелец, клан удалится)
    ...
    async def clan_join_command(
        ...
        if not context.args:
            await update.message.reply_text(
                "❌ Используйте: /clan_join <название_клана>"
            )
            return
...
💡 Вы можете создать новый клан или вступить в другой.
    ...
    async def admin_health_command(
        ...
        await admin_health_command(
            update, context, self.admin_system, self.monitoring_system
        )
    ...
    async def admin_errors_command(
        ...
        await admin_errors_command(
            update, context, self.admin_system, self.error_handling_system
        )
    ...
    async def admin_backup_command(
        ...
        await admin_backup_command(
            update, context, self.admin_system, self.backup_system
        )
    ...
    async def admin_background_status_command(
        ...
        await admin_background_status_command(
            update, context, self.admin_system, self.background_task_manager
        )
    ...
    async def admin_parsing_reload_command(
        ...
        await admin_parsing_reload_command(
            update, context, self.admin_system, self.message_parser
        )
